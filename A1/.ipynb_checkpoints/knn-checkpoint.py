...
def compute_distances_two_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    # Calculate squared Euclidean distances using nested loops
...
def compute_distances_one_loop(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    dists = x_train.new_zeros(num_train, num_test)
    ##########################################################################
    # TODO: Implement this function using only a sinijiogle loop over x_train.   #
    #                                                                        #
    # You may not use torch.norm (or its instance method variant), nor any   #
    # functions from torch.nn or torch.nn.functional.                        #
    ##########################################################################
    ...
    x_test_flat = x_test.view(num_test, -1)    # Shape: (num_test, D)
    for i in range(num_train):
        # Compute the squared distances for the i-th training example
        # Using broadcasting to calculate the distances
...
def compute_distances_no_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    xtrain_fla = x_train.view(num_train , -1) 
    xtest_fla = x_test.view(num_test, -1) 

    train_sq = (xtrain_fla ** 2).sum(dim = 1).view(-1 , 1)
    test_sq = (xtest_fla** 2).sum(dim = 1).view(1 , -1)
    product2 = torch.mm(xtrain_fla , xtest_fla.t()) * 2
    dists = train_sq + test_sq - product2
...
def predict_labels(dists: torch.Tensor, y_train: torch.Tensor, k: int = 1):
    ...
    for i in range(num_test) :
        ...
        label = y_train[idx]
        uni_label , cnt = torch.unique(label , return_counts = True)
        candi = uni_label[cnt == cnt.max()]
        y_pred[i] = candi.min()
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
...
def knn_cross_validate(
    ...
    for k in k_choices:
        accuracies = []
        for validation_idx in range(num_folds) :
            trainset = torch.cat(x_train_folds[0:validation_idx] + x_train_folds[validation_idx+1:])
            trainset_label = torch.cat(y_train_folds[0:validation_idx] + y_train_folds[validation_idx+1:])
            classifier = KnnClassifier(trainset , trainset_label)

            accuracies_val = classifier.check_accuracy(x_train_folds[validation_idx] , y_train_folds[validation_idx] , k)
            accuracies.append(accuracies_val)
...
def knn_get_best_k(k_to_accuracies: Dict[int, List]):
    ...
    for k , acc in k_to_accuracies.items():
        if mx_mean < sum(acc) / len(acc) : 
            mx_mean = sum(acc) / len(acc)
            best_k = k
        elif abs(sum(acc) / len(acc) - mx_mean) < 1e-7:
            best_k = min(best_k , k)
