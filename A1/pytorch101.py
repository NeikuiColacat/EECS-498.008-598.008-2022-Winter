...
def mutate_tensor(
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    for (u , v) , val in zip(indices , values) : x[u,v] = val
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return x
...
def count_tensor_elements(x: Tensor) -> int:
    ...
    # Replace "pass" statement with your code
    num_elements = 1
    for i in x.shape : num_elements *= i
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return num_elements
...
def multiples_of_ten(start: int, stop: int) -> Tensor:
    ...
    # Replace "pass" statement with your code
    st = (start + 9) // 10 * 10
    if st <= stop : x = torch.arange(st , stop , step = 10 , dtype=torch.float64)
    else : x = torch.empty(0 , dtype=torch.float64)
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return x
...
def slice_assignment_practice(x: Tensor) -> Tensor:
    ...
    x[2 : 4,  0 : 2] = x[2 : 4 , 2 : 4] = torch.tensor([[3,4] , [3,4]])
    x[2 : 4, 4 : 6] = 5
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
...
def reshape_practice(x: Tensor) -> Tensor:
    ...
    ##########################################################################
    rx = x.reshape(3,8)
    y = torch.zeros(3 , 8 , dtype=x.dtype)
    y[0 , :4] = rx[0 , :4]
    y[0 , 4:] = rx[1 , 4:]
    y[1 , :4] = rx[0,4:]
    y[1, 4:] = rx[2 , :4]
...
def normalize_columns(x: Tensor) -> Tensor:
    ...
    variance = ((x - mu) ** 2).sum(dim = 0 , keepdim=True) / (x.shape[0]-1)
    sigma = variance.sqrt()
    y = (x - mu) / sigma
...
def challenge_mean_tensors(xs: List[Tensor], ls: Tensor) -> Tensor:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    stk = torch.nn.utils.rnn.pad_sequence(xs , batch_first=True)
