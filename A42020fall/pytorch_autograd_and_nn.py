...
def initialize_three_layer_conv_part2(dtype=torch.float, device='cpu'):
	...
	tmp = {'dtype': dtype, 'device': device}

	conv_w1 = nn.init.kaiming_normal_(torch.empty(
		channel_1, C, kernel_size_1, kernel_size_1, requires_grad=True, **tmp))
	conv_b1 = torch.zeros(channel_1, requires_grad=True, **tmp)

	H = (H - kernel_size_1 + 2 * 2) / 1 + 1
	W = (W - kernel_size_1 + 2 * 2) / 1 + 1

	conv_w2 = nn.init.kaiming_normal_(torch.empty(
		channel_2, channel_1, kernel_size_2, kernel_size_2, requires_grad=True, **tmp))
	conv_b2 = torch.zeros(channel_2,requires_grad=True,**tmp)

	H = (H - kernel_size_2 + 2 * 1) / 1 + 1
	W = (W - kernel_size_2 + 2 * 1) / 1 + 1

	fc_w = nn.init.kaiming_normal_(torch.empty(
		num_classes, int(channel_2*H*W), requires_grad=True, **tmp))
	fc_b = torch.zeros(num_classes,requires_grad=True,**tmp)
...
class ResidualBottleneckBlock(nn.Module):
	def __init__(self, Cin, Cout, downsample=False):
		...
		self.block = nn.Sequential(
			nn.BatchNorm2d(Cin),
			nn.ReLU(),
			nn.Conv2d(Cin,Cout//4,1,2 if downsample else 1),
