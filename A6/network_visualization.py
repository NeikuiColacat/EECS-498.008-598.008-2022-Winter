...
def make_adversarial_attack(X, target_y, model, max_iter=100, verbose=True):
    ...
    for i in range(max_iter):
        pred_score = model(X_adv)
        _ , pred_class = torch.max(pred_score,dim=1)
        if pred_class == target_y : break

        loss = pred_score[:,target_y]
        loss.backward()

        dX = learning_rate * X_adv.grad / ((X_adv.grad ** 2).sum() ** 0.5)

        with torch.no_grad():
            X_adv += dX

        X_adv.grad.zero_()
        print(f'iter : {i} , target_score {pred_score[:,target_y]} , max score {_}')
    ##############################################################################
    #                             END OF YOUR CODE                               #
    ##############################################################################
    return X_adv
...
def class_visualization_step(img, target_y, model, **kwargs):
    ...
    loss = pred_score[:,target_y] - l2_reg * torch.sum(img**2)
    loss.backward()  
